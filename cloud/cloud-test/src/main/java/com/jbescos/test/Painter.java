...
public class Painter extends JFrame {
    ...
    private static final double EWMA = Utils.EWMA_CONSTANT;
        ...
        addMouseMotionListener(new MouseMotionAdapter() {
            public void mouseDragged(MouseEvent event) {
                if (event.getX() > x) {
                    ...
                    CSV.append(Utils.fromDate(Utils.FORMAT_SECOND, new Date(newDate))).append(",").append("SYMBOL")
                            .append(",").append(price).append(",").append(previousResult).append("\r\n");
                    repaint();
                }
            }
        });
