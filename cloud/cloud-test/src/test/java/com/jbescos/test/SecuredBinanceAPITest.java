...
public class SecuredBinanceAPITest {
    ...
    public void checkExploits() throws InvalidKeyException, NoSuchAlgorithmException, IOException {
        ...
        for (String symbol : symbols) {
            try {
                System.out.println(symbol + " Wallet: " + kucoinWallet.get(symbol) + " Kucoin Address:"
                        + kucoin.depositAddress(symbol, kucoinSellPrices));
            } catch (Exception e) {
                System.out.println(
                        symbol + " Wallet: " + kucoinWallet.get(symbol) + " Kucoin Addresss: " + e.getMessage());
            }
            try {
                System.out.println(symbol + " Wallet: " + binanceWallet.get(symbol) + " Binance Address:"
                        + binance.depositAddress(symbol));
            } catch (Exception e) {
                System.out.println(
                        symbol + " Wallet: " + binanceWallet.get(symbol) + " Binance Address: " + e.getMessage());
            }
        }
        client.close();
    }
    ...
    public void signature() throws InvalidKeyException, NoSuchAlgorithmException {
        Client client = ClientBuilder.newClient();
        SecuredBinanceAPI api = SecuredBinanceAPI.create(client,
