...
public class CsvProfitRow {
    ...
    public static final String HEADER = "SELL_DATE,FIRST_BUY_DATE,SYMBOL,QUANTITY_BUY,QUANTITY_SELL,QUANTITY_USDT_BUY,QUANTITY_USDT_SELL,COMMISSION_%,COMMISSION_USDT,USDT_PROFIT,NET_USDT_PROFIT,PROFIT_%,BUY_IDS,SELL_ID,SYNC"
            + Utils.NEW_LINE;
    ...
    public CsvProfitRow(CsvProfitRow old, String quantityBuy, String quantityUsdtBuy, String quantitySell,
            String quantityUsdtSell, String commissionUsdt, String usdtProfit, String netUsdtProfit,
            String profitPercentage, String commissionPercentage) {
        this.firstBuyDate = old.firstBuyDate;
        this.sellDate = old.sellDate;
        ...
        this.netUsdtProfit = netUsdtProfit;
        this.profitPercentage = profitPercentage;
        this.buyIds = old.buyIds;
        this.sellId = old.sellId;
        this.sync = true;
    }

    public String toCsvLine() {
        StringBuilder data = new StringBuilder();
        data.append(Utils.fromDate(Utils.FORMAT_SECOND, sellDate)).append(",")
                .append(Utils.fromDate(Utils.FORMAT_SECOND, firstBuyDate)).append(",").append(symbol).append(",")
                .append(quantityBuy).append(",").append(quantitySell).append(",").append(quantityUsdtBuy).append(",")
                .append(quantityUsdtSell).append(",").append(commissionPercentage).append(",").append(commissionUsdt)
                .append(",").append(usdtProfit).append(",").append(netUsdtProfit).append(",").append(profitPercentage)
                .append(",").append(buyIds).append(",").append(sellId).append(",").append(sync).append(Utils.NEW_LINE);
    ...
    public static CsvProfitRow build(String commission, TransactionsSummary summary, CsvTransactionRow sell) {
        if (!summary.getPreviousBuys().isEmpty()) {
            ...
            /*
             * Normalize SELL quantity to match the BUY quantity. It could happen that the
             * user did a purchase out of the system. It takes the current price from newest
             */
            ...
            /*
             * quantityUsdtBuy -> 100 usdtProfit -> X
             * 
             * profitPercentage = usdtProfit * 100 / quantityUsdtBuy
             */
            String profitPercentage = Utils
                    .format(usdtProfit.multiply(d100).divide(quantityUsdtBuy, 8, RoundingMode.DOWN), 2) + "%";
            StringBuilder buyIds = new StringBuilder();
            for (CsvTransactionRow buy : summary.getPreviousBuys()) {
                if (buyIds.length() != 0) {
                    buyIds.append(Utils.ORDER_ID_SPLIT);
                }
                buyIds.append(buy.getOrderId());
            }
            return new CsvProfitRow(firstBuyDate, sellDate, symbol, Utils.format(quantityBuy, 2),
                    Utils.format(quantityUsdtBuy, 2), quantitySell, quantityUsdtSell, commissionPercentage,
                    Utils.format(commissionUsdt, 2), Utils.format(usdtProfit, 2), Utils.format(netUsdtProfit, 2),
                    profitPercentage, buyIds.toString(), sell.getOrderId());
        } else {
            return null;
        }
    }
    ...
    public String toString() {
        StringBuilder content = new StringBuilder(symbol).append(" SELL ")
                .append(Utils.fromDate(Utils.FORMAT_SECOND, sellDate)).append("\nFirst purchase: ")
                .append(Utils.fromDate(Utils.FORMAT_SECOND, firstBuyDate)).append("\nBuy / Sell: ")
                .append(quantityUsdtBuy).append("$ / ").append(quantityUsdtSell).append("$").append("\nProfit: ")
                .append(usdtProfit).append("$ (<b>").append(profitPercentage).append("</b>)");
        if (Double.parseDouble(usdtProfit) < 0) {
            content.append(" ❌");
        } else {
            content.append(" ✅");
        }
