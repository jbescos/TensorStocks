...
public class Main {
    ...
    public static void main(String[] args)
        ...
        Function<String, MessageWorker<BinanceMessage>> binanceWorker = symbol -> new PricesWorker<>(symbol,
                binancePrices);
        IWebsocket binanceSocket = new BinanceWebSocket(
                new MessageHandlerImpl<BinanceMessage>(BinanceMessage.class, binanceWorker),
                BinanceWebSocket.SUBSCRIPTION_BOOK_TICKER);
        ConcurrentHashMap<String, Price> kucoinPrices = new ConcurrentHashMap<>();
        Client client = ClientBuilder.newClient();
        PublicAPI publicAPI = new PublicAPI(client);
        Function<String, MessageWorker<KucoinMessage>> kucoinWorker = symbol -> new PricesWorker<>(symbol,
                kucoinPrices);
        IWebsocket kucoinSocket = new KucoinWebsocket(
                new MessageHandlerImpl<KucoinMessage>(KucoinMessage.class, kucoinWorker), client);
        binanceSocket.start();
        kucoinSocket.start();
...
//					LOGGER.warning("No symbol found: " + symbol);
                    ...
                    if (delta < 2000 && delta > -2000) {
                        ...
                        if (benefit(binanceSellPrice, kucoinBuyPrice)) {
                            ...
                            if (profit > Constants.PROFIT) {
                                ...
                                if (trade) {
                                    LOGGER.info(symbol + ": buy in Kucoin and sell in Binance. Profit "
                                            + format(profit, 8) + "%");
                                }
                            }
                        }
                        ...
                        if (benefit(kucoinSellPrice, binanceBuyPrice)) {
                            ...
                            if (profit > Constants.PROFIT) {
                                boolean trade = verifySymbolForTrading(publicAPI, tempDisabledSymbols, symbol);
                                if (trade) {
                                    LOGGER.info(symbol + ": buy in Binance and sell in Kucoin. Profit "
                                            + format(profit, 8) + "%");
                                }
                            }
                        }
    ...
    // Check whether the symbol is available for trading and it was not processed
    // few minutes ago
    private static boolean verifySymbolForTrading(PublicAPI publicAPI, Map<String, Long> tempDisabledSymbols,
            String symbol) {
